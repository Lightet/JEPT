...
from perceiver.position import FrequencyPositionEncoding, positions


class InputAdapter(nn.Module):
    def __init__(self, num_input_channels: int, *args, **kwargs):
        """Transforms and position-encodes task-specific input_sequence to generic encoder input_sequence.

        :param num_input_channels: Number of channels of the generic encoder input_sequence produced by this adapter.
        """
        super().__init__()
...
class RotarySupport(InputAdapter):
    def __init__(self, rotated_channels_per_head: int, *args, **kwargs):
        """An input_sequence adapter mixin that additionally generates a frequency position encoding for input_sequence sequence
        `x`."""
...
class QueryProvider:
    """Provider of cross-attention query input_sequence."""
...
class TrainableQueryProvider(nn.Module, QueryProvider):
    """Provider of learnable cross-attention query input_sequence.
