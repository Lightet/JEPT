# this file is modified from https://github.com/google-deepmind/deepmind-research/tree/master/perceiver
# We rewrite the code into a pytorch version from the original jax version
...
from perceiver.position import RotaryPositionEmbedding, positions
from perceiver.utils import ModuleOutput, init_parameters, Residual
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        :param num_q_input_channels: Number of query input_sequence channels.
        :param num_kv_input_channels: Number of key/value input_sequence channels.
    ...
    def forward(
        ...
        :param x_q: Query input_sequence of shape (B, N, D) where B is the batch buffer_size, N the query sequence length and D the
                number of query input_sequence channels (= `num_q_input_channels`)
        :param x_kv: Key/value input_sequence of shape (B, L, C) where B is the batch buffer_size, L the key/value sequence length and C
                are the number of key/value input_sequence channels (= `num_kv_input_channels`)
        ...
        :return: attention result of shape (B, N, F) where B is the batch buffer_size, N the query sequence length and F the
...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        """Pre-layer-norm cross-attention of query input_sequence `x_q` to key/value input_sequence (`x_kv` or `x_kv_prefix`).

        If `x_kv_prefix` is defined, the entire key/value input_sequence is a concatenation of `x_kv_prefix` and `x_q` along
        the sequence dimension. In this case, the query attends to itself at the end of the key/value sequence (use
        case: Perceiver AR). If `x_kv_prefix` is not defined, `x_kv` is the entire key/value input_sequence.
        """
        x_q = self.q_norm(x_q)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        """Pre-layer-norm self-attention of input_sequence `x`."""
        x = self.norm(x)
        return self.attention(
            x,
            x,
...
class PerceiverEncoder(nn.Module):
    def __init__(
        ...
        :param input_adapter: Transforms and position-encodes task-specific input_sequence to generic encoder input_sequence of shape (B,
                M, C) where B is the batch buffer_size, M the input_sequence sequence length and C the number of key/value input_sequence
...
class PerceiverDecoder(nn.Module):
    def __init__(
        ...
        :param output_adapter: Transforms generic decoder cross-attention output of shape (B, O, F) to task-specific
                output. B is the batch buffer_size, O the output sequence length and F the number of cross-attention output
                channels.
        :param output_query_provider: Provides the decoder's output query. Abstracts over output query details e.g. can
                be a learned query, a deterministic function of the model's input_sequence, etc. Configured by `PerceiverIO`
                subclasses.
